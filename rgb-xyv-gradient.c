...
void gradient_xyv(image im, couleur_xyv c_left, couleur_xyv c_right);
...
void gradient_xyv(image im, couleur_xyv c_left, couleur_xyv c_right)
{
  ...
  for (x = 0; x < im.largeur; x++)
  {
    ...
    couleur_rgb c_rgb;
    double proportion = x * 1.0 / (im.largeur - 1);
    c_xyv = interpolation_xyv(proportion, c_left, c_right);
    c_rgb = xyv2rgb(c_xyv);
...
void usage(char *nom_programme, char *message)
{
  ...
  fprintf(stderr, "\n");
  fprintf(stderr, "Usage: %s result.ppm LxH r1,v1,b1 r2,v2,b2 algo\n", nom_programme);
  fprintf(stderr, "\n");
...
{
  int largeur, hauteur;
  couleur_rgb c_rgb_gauche, c_rgb_droite;
  couleur_xyv c_xyv_gauche, c_xyv_droite;
  ...
  c_rgb_gauche = argv_vers_couleur_rgb(argv[0], argv[3]);
  c_rgb_droite = argv_vers_couleur_rgb(argv[0], argv[4]);

  sscanf(argv[5], "%d", &type_color);
  if (type_color == 0)
  {
    // extraction des deux couleurs
    gradient_rgb(im, c_rgb_gauche, c_rgb_droite);
  }
  else if (type_color == 1)
  {
    c_xyv_gauche = rgb2xyv(c_rgb_gauche);
    c_xyv_droite = rgb2xyv(c_rgb_droite);
    gradient_xyv(im, c_xyv_gauche, c_xyv_droite);
  }
